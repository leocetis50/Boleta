...
lista_alumnos = ft.dropdown(
    width = 300,
    label = "Alumnos",
    options = [ft.dropdown.Option("Ismael Macias Perez"),
               ft.dropdawn.option("Ayari Cosme Rodriguez"),
               ft.dropdawn.option("Leonardo Martinez Martinez"),
               ft.dropdawn.option("Betsabeth Guzman Tellez"),
               ft.dropdawn.option("Elisabeth"),
               ft.dropdawn.option("Santiago Bello Perez"),
               ],
)
...
mat = ft.dropdown(width=200,label="Matematicas",
                  options= [ft.dropdown.Option(str(i)) for i in range(10,101,10)])
ing = ft.dropdown(widt=200,label="Ingles",
                  options=[ft.dropdown.Option(str(i)) for i in range (10,101,10)])
info = ft.dropdown(width=200, label= "Informatica",
                   options=[ft.dropdown.Option(str(i)) for i in range (10,101,10)])
hist = ft.dropdown(width=200,label="Historia",
                   options=[ft.dropdown.Option(str(i)) for i in range(10,101,10)])
quim = ft.dropdown(width=200,label= "Quimica",
                   options=[ft.dropdown.Option(str(i)) for i in range(10,101,10)])
huma = ft.dropdown(widt=200,label="Humanidades",
                   options=[ft.dropdown.Option(str(i)) for i in range(10,101,10)])
